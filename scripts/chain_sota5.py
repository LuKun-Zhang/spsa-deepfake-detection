...
ST   = "/root/autodl-tmp/chain_sota5.status"
...
log("chain5 start")
p0 = launch(["srm"])
log("launched srm as 3-up probe")
...
if remain_min() >= 170:
    log("GROUP A 3-up capsule_net,sia,ffd  remain=%dmin" % remain_min())
    pg = launch(["capsule_net","sia","ffd"]); wait_ps(pg)
    log("group A done")
else:
    log("SKIP group A remain=%dmin" % remain_min())

if remain_min() >= 135:
    log("GROUP B 2-up iid,rfm  remain=%dmin" % remain_min())
    pg = launch(["iid","rfm"]); wait_ps(pg)
    log("group B done")
else:
    log("SKIP group B remain=%dmin" % remain_min())
