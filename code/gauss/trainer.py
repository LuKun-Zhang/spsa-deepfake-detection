...
class Trainer(object):
    ...
    def train_epoch(
        ...
        for iteration, data_dict in tqdm(enumerate(train_data_loader),total=len(train_data_loader)):
            ...
            if iteration % 300 == 0 and self.config['local_rank']==0:
                if self.config['SWA'] and (epoch>self.config['swa_start'] or self.config['dry_run']) and isinstance(self.scheduler, SWALR):
                    self.scheduler.step()
                # info for loss
    ...
    def test_epoch(self, epoch, iteration, test_data_loaders, step):
        # [repro patch 01/01] 7-point checkpoint dump: save an independent weight at each validation step.
        # Banked for cross-domain eval / trajectory analysis. Does NOT alter any training logic.
        if step >= 0:
            try:
                _m = self.model.module if hasattr(self.model, 'module') else self.model
                torch.save(_m.state_dict(), os.path.join(self.log_dir, 'ckpt_step{}.pth'.format(step)))
                self.logger.info('[repro] saved ckpt_step{}.pth'.format(step))
            except Exception as _e:
                self.logger.warning('[repro] ckpt_step save FAILED: {}'.format(_e))
