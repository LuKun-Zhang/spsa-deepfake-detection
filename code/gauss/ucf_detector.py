...
from .spsa_modules import SPSA, GaussMix
...
class UCFDetector(AbstractDetector):
    def __init__(self, config):
        ...
        self.con_gan = Conditional_UNet(
            use_spsa=config.get('spsa', False),
            use_gauss=config.get('gauss', False),
            gauss_amp=config.get('gauss_amp', 0.1),
            spsa_off_step=config.get('spsa_off_step', None),
            spsa_on_step=config.get('spsa_on_step', None),
        )
        # [night_run] 训练步计数器（E3.1a/E3.1b 旁路判定用；每次训练 forward 恰好 +1）
        self._train_steps = 0

        # SPSA2: SPSA on the common fingerprint (f_share) before head_sha
        self.use_spsa2 = config.get('spsa2', False)
        if self.use_spsa2:
            self.spsa2 = SPSA(256, 256, e=2.0)
    ...
    def build_loss(self, config):
        ...
        # ===== Cross-Method Consistency (our innovation) =====
        if config.get('cross_method_consistency', False):
            cons2_loss_class = LOSSFUNC[config['loss_func']['cons2_loss']]
            loss_func['cons2'] = cons2_loss_class(temperature=config.get('consistency_temperature', 0.1))
        return loss_func
    ...
    def get_train_losses(self, data_dict: dict, pred_dict: dict) -> dict:
        ...
        cat_data = data_dict['image']
        # 'image' always contains [real, fake]; fake2 lives in a separate key
        ...
        # 6. cross-method consistency loss (our innovation, warmup-gated by cons_start_epoch)
        cons_loss = pred_dict.get('cons_loss')
        cons_start = self.config.get('cons_start_epoch', 0)
        if cons_loss is not None and getattr(self, 'epoch', 0) >= cons_start:
            loss = loss + self.config.get('consistency_weight', 0.05) * cons_loss
        loss_dict = {
            'overall': loss,
            'common': loss_sha,
            'specific': loss_spe,
    ...
    def forward(self, data_dict: dict, inference=False) -> dict:
        ...
        bs = f_share.size(0)
        # [night_run] 训练步计数并注入 con_gan（SPSA 旁路判定）
        # 注意：计数器放顶层 forward —— con_gan 每训练步被调用 4 次（自重建×2 + 交叉重建×2），
        # 若放模块内部会把步数放大 4 倍导致旁路时机错误；此处每训练 batch 恰好 +1。
        # 推理分支（inference=True）在上方提前 return，不经过此处，eval 不计数、不旁路。
        if self.training:
            self._train_steps += 1
            self.con_gan.cur_step = self._train_steps
        # ===== Cross-Method Consistency (our innovation) =====
        # fake2 (same-source different-method fake frames) is kept in a separate
        # key 'fake2_image' instead of being concatenated into 'image', so run
        # it through the encoder independently to obtain its common fingerprint.
        # Compute the consistency loss on the common fingerprints BEFORE the
        # idx-aug shuffle (which would destroy the anchor-positive correspondence).
        cons_loss = None
        if (self.config.get('cross_method_consistency', False) and data_dict.get('fake2_image') is not None
                and getattr(self, 'epoch', 0) >= self.config.get('cons_start_epoch', 0)):
            n = bs // 2  # half of the [real, fake] main batch
            f2_all = self.encoder_f.features(data_dict['fake2_image'])
            f_share2 = self.block_sha(f2_all)
            # anchor: fake part (second half); positive: same-source fake2 part
            anchor = self.pool(f_share[n:]).view(n, -1)
            positive = self.pool(f_share2).view(n, -1)
            cons_loss = self.loss_func['cons2'](anchor, positive)
...
class Conditional_UNet(nn.Module):
    ...
    def __init__(self, use_spsa=False, use_gauss=False, gauss_amp=0.1,
                 spsa_off_step=None, spsa_on_step=None):
        ...
        # [night_run] 旁路配置：spsa_off_step（晚段关）/ spsa_on_step（只开晚期）
        self.use_gauss = use_gauss
        self.spsa_off_step = spsa_off_step
        self.spsa_on_step = spsa_on_step
        self.cur_step = 0  # UCFDetector 每训练步注入（见 UCFDetector.forward）
        if use_spsa:
            self.spsa = SPSA(128, 128)
        if use_gauss:
            self.gauss = GaussMix(amp=gauss_amp)
    ...
    def forward(self, c, x):  # c is the style and x is the content
        ...
        x = self.dconv_up2(x)
        if self.use_spsa:
            # E3.1a：spsa_off_step 之后旁路（训练时关，验证/部署不受影响）
            # E3.1b：spsa_on_step 之前旁路（只开晚期）
            bypass = (
                (self.spsa_off_step is not None and self.cur_step >= self.spsa_off_step)
                or (self.spsa_on_step is not None and self.cur_step < self.spsa_on_step)
            )
            if not bypass:
                x = self.spsa(x)
        if self.use_gauss:
