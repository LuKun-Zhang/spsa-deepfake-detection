...
class pairDataset(DeepfakeAbstractBaseDataset):
    def __init__(self, config=None, mode='train'):
        ...
        # Build a mapping: source video ID -> list of fake items (img, spe_label, 1)
        # so that for each fake frame we can sample a "same-source, different-method"
        # frame as the positive pair of the consistency loss.
        self.consistency = config.get('cross_method_consistency', False) and (mode == 'train')
        self.fake_vid_dict = None
        if self.consistency:
            self.fake_vid_dict = defaultdict(list)
            for item in self.fake_imglist:
                vid = self._extract_vid(item[0])
                if vid is not None:
                    self.fake_vid_dict[vid].append(item)

    @staticmethod
    def _extract_vid(path):
        # path like: .../frames/971_564/000.png  -> return '971_564'
        p = path.replace('\\', '/')
        marker = '/frames/'
        idx = p.find(marker)
        if idx == -1:
            return None
        return p[idx + len(marker):].split('/')[0]
    ...
    def __getitem__(self, index, norm=True):
        ...
        # ===== Cross-Method Consistency: sample same-source different-method fake =====
        fake2_image_trans = None
        if self.consistency:
            vid = self._extract_vid(fake_image_path)
            cands = [it for it in self.fake_vid_dict.get(vid, []) if it[1] != fake_spe_label]
            if cands:
                fake2_path, _, _ = random.choice(cands)
            else:
                fake2_path = fake_image_path  # fallback: use self (contributes ~0 loss)
            fake2_image = np.array(self.load_rgb(fake2_path))
            fake2_image_trans, _, _ = self.data_aug(fake2_image, None, None)

        if not norm:
            return {"fake": (fake_image_trans, fake_label), 
                    "real": (real_image_trans, real_label),
                    "fake2": fake2_image_trans}
        ...
        if fake2_image_trans is not None:
            fake2_image_trans = self.normalize(self.to_tensor(fake2_image_trans))
    ...
    def collate_fn(batch):
        ...
        fake2_images = [data["fake2"] for data in batch]
        ...
        # Cross-method consistency: stack the same-source different-method fake images
        if fake2_images[0] is not None:
            fake2_images = torch.stack(fake2_images, dim=0)
        else:
            fake2_images = None
